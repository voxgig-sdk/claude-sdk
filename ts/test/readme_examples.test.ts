...
//     (\`new ClaudeSDK(...)\` / \`ClaudeSDK.test(...)\` become
//     \`ClaudeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"message":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ClaudeSDK(...)\` and any
// \`ClaudeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ClaudeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ClaudeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
