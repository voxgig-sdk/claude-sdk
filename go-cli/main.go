// Command Claude-cli is a CLI + REPL for the Claude SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CLAUDE_APIKEY (api key), CLAUDE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CLAUDE_APIKEY carries the API key and
	// CLAUDE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CLAUDE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CLAUDE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewClaudeSDK(opts)
