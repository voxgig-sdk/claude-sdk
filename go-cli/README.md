...
boru-driven command-line client **and** interactive REPL for the Claude
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/claude-cli)
make build
...
CLAUDE_BASE=https://api.example.com ./claude-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/claude-cli
   ```
...
Run with no arguments to open a REPL (prompt `claude>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `message`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/claude-cli`. |
...
(`./claude-cli <expr>`) and interactively (the REPL), and expressions compose the
