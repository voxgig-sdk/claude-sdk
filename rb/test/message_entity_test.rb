...
class MessageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLAUDE_TEST_MESSAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def message_basic_setup(extra)
  ...
  entid_env_raw = ENV["CLAUDE_TEST_MESSAGE_ENTID"]
  ...
  if env["CLAUDE_TEST_LIVE"] == "TRUE"
    ...
    client = ClaudeSDK.new(Helpers.to_map(merged_opts))
  end
