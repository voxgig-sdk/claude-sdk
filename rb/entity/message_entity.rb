...
class MessageEntity
  ...
  def data_set(args)
    if args
      @_data = ClaudeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ClaudeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Message, Hash] the created Message; raises ClaudeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ClaudeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
