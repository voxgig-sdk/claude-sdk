# Claude SDK test runner

from __future__ import annotations
import os
import json

from claude_sdk.utility.voxgig_struct import voxgig_struct as vs
...
class ClaudeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ClaudeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ClaudeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ClaudeTestRunner.getenv("CLAUDE_TEST_LIVE")
        override = ClaudeTestRunner.getenv("CLAUDE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ClaudeTestRunner.getenv(key)
        ...
        explain = ClaudeTestRunner.getenv("CLAUDE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CLAUDE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ClaudeTestRunner._test_control is not None:
            return ClaudeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ClaudeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ClaudeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ClaudeTestRunner.load_test_control()
...
def load_env_local():
    ClaudeTestRunner.load_env_local()


def env_override(m):
    return ClaudeTestRunner.env_override(m)


def entity_data(v):
    return ClaudeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ClaudeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ClaudeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ClaudeTestRunner.load_test_control()


def live_client_options():
    return ClaudeTestRunner.live_client_options()


def live_delay_ms():
    return ClaudeTestRunner.live_delay_ms()
