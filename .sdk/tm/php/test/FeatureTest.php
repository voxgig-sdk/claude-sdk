...
class FtCtrl extends ClaudeControl
{
...
class FtClient
{
    ...
    private ClaudeUtility $utility;

    public function __construct(ClaudeUtility $utility, array $options)
    {
    ...
    public function get_utility(): ClaudeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ClaudeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ClaudeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ClaudeConfig::make_feature($name);
    ...
    private static function buildUrl(ClaudeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ClaudeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ClaudeResponse($fetched);
        }
    ...
    private function fail(ClaudeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ClaudeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
