...
//    only - a typed ClaudeContext there would make every ctx
...
final class ClaudeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ClaudeContext($clean, null);
    ...
    public static function err_from_map($m): ?ClaudeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ClaudeError(is_string($code) ? $code : '', $msg);
    }
