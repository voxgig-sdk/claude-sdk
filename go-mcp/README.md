...
[MCP](https://modelcontextprotocol.io) server exposing the Claude SDK as
two agent tools — `claude_list` and `claude_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/claude-mcp)
make build

# 2. Provide credentials via the environment
export CLAUDE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user claude \
  -- /absolute/path/to/claude-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./claude-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// claude_list: first page of records
{ "entity": "message" }
{ "entity": "message", "query": { } }

// claude_load: one record by id
{ "entity": "message", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/claude-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user claude \
     -- "$PWD"/dist/*/claude-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `claude_list` and `claude_load` tools now appear
   in new sessions. Ask the agent to *"list message using claude"*
   and it calls `claude_list` with `{"entity":"message"}`.
...
```sh
export CLAUDE_APIKEY=sk_live_xxx            # API key
export CLAUDE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `claude_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `claude_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| `CLAUDE_BASE` | Optional override of the API base URL. |
...
```sh
./claude-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"claude_load","arguments":{"entity":"message","query":{"id":1}}}}'
```
