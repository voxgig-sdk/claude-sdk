...
MCP server exposing the Claude SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `claude_list` | `entity`, optional `query` map | First page of records as JSON |
| `claude_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `claude_list` and
`claude_load` tools become available in new sessions.
