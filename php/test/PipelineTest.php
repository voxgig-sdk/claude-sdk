...
{
    private static function utility(): ClaudeUtility
    {
        return new ClaudeUtility();
    }
    ...
    private static function ctx(array $over = []): ClaudeContext
    {
        ...
        $ctx->op = new ClaudeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ClaudeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ClaudeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ClaudeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ClaudeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ClaudeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ClaudeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ClaudeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ClaudeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ClaudeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = ClaudeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ClaudeSpec(['step' => 's']),
            'response' => new ClaudeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ClaudeResult([]),
        ]);
        [, $err] = ClaudeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ClaudeSpec(['step' => 's']),
            'response' => new ClaudeResponse(self::resp(200, ['v' => 1])),
            'result' => new ClaudeResult([]),
        ]);
        [, $err] = ClaudeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ClaudeSpec(['step' => 's']),
            'response' => new ClaudeResponse(self::resp(200, ['v' => 2])),
            'result' => new ClaudeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ClaudeMakeResponse::call($ctx);
    ...
    {
        $preset = new ClaudeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ClaudeSpec([]),
            'response' => new ClaudeResponse([]),
            'result' => new ClaudeResult([]),
        ...
        [$got, $err] = ClaudeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ClaudeResult([])]);
        [, $err] = ClaudeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ClaudeSpec([]), 'result' => null]);
        [, $err] = ClaudeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ClaudeSpec(['step' => 's']),
            'result' => new ClaudeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ClaudeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ClaudeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ClaudeSpec(['step' => 's']),
            'result' => new ClaudeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ClaudeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ClaudeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ClaudeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ClaudeSpec([]),
            'result' => new ClaudeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ClaudeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ClaudeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ClaudeError('boom', 'boom');
        $utility->fetcher = function (ClaudeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ClaudeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ClaudeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ClaudeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ClaudeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ClaudeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ClaudeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ClaudeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ClaudeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ClaudeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ClaudeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ClaudeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ClaudeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ClaudeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ClaudeResult(['ok' => false])]);
        $this->expectException(ClaudeError::class);
        ClaudeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ClaudeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ClaudeMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ClaudeBaseFeature();
        $b = new ClaudeBaseFeature();
        ClaudeFeatureAdd::call($ctx, $a);
        ClaudeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ClaudeBaseFeature
    {
        $f = new ClaudeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ClaudeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ClaudeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ClaudeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ClaudePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ClaudeResponse(['headers' => null]),
            'result' => new ClaudeResult([]),
        ]);
        ClaudeResultHeaders::call($ctx);
