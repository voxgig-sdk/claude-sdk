...
class ClaudeMakeError
{
    public static function call(?ClaudeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ClaudeContext([], null);
        }
        $op = $ctx->op ?? new ClaudeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ClaudeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ClaudeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ClaudeError('', $msg, $ctx);
