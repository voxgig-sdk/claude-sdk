...
describe("MessageEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLAUDE_TEST_MESSAGE_ENTID JSON to run live")
      return
    end
...
function message_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CLAUDE_TEST_MESSAGE_ENTID")
  ...
  if env["CLAUDE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["CLAUDE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
