...
import (
	...
	omni "github.com/voxgig-sdk/claude-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ClaudeError); is {
				out := map[string]any{
					"name":    "ClaudeError",
...
func omniCtx(arg any, client *sdk.ClaudeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ClaudeSDK); is {
			client = live
